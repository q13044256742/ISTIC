    ...
    public partial class Frm_MyWork : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void TreeView_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            if(e.Button == MouseButtons.Left)
            {
                ...
                else if(type == ControlType.Subject)
                {
                    ...
                    {
                        tab_MenuList.TabPages.Clear();
                        object topId = SqlHelper.ExecuteOnlyOneQuery($"SELECT ti_id FROM topic_info WHERE ti_id=(SELECT si_obj_id FROM subject_info WHERE si_id='{e.Node.Name}')");
                        object _tempParam = SqlHelper.ExecuteOnlyOneQuery($"SELECT pi_id FROM project_info WHERE pi_id='{topId}'");
                        if(_tempParam != null)
                        {
                            ShowTab("plan", 0);
                            LoadPlanPage(e.Node.Parent.Parent.Parent.Name, e.Node.Parent.Parent.Parent.ForeColor);
        ...
        private void LoadFileBoxTableInstance(ListView leftView, ListView rightView, string key, object pbId, object objId)
        {
            ...
            string querySql = $"SELECT pfl_id, dd_name, pfl_name, pfl_date FROM processing_file_list LEFT JOIN data_dictionary " +
                $"ON pfl_categor=dd_id WHERE pfl_obj_id = '{objId}' AND pfl_status={(int)GuiDangStatus.NonGuiDang} ORDER BY dd_name, pfl_date";
            DataTable dataTable = SqlHelper.ExecuteQuery(querySql);
            for(int i = 0; i < dataTable.Rows.Count; i++)
            {
        ...
        private void Cbo_Box_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            {
                object pbId = comboBox.SelectedValue;
                LoadFileBoxTable(pbId, dgv_Project_FileList.Tag, ControlType.Project);
            }
        ...
        private void LoadPageBasicInfo(object projectId, ControlType type, System.Drawing.Color color)
        {
            ...
            else if(type == ControlType.Project)
            {
                ...
                if(isBacked)
                {
                    btn_Topic_QTReason.Text = $"质检意见({GetAdvincesAmount(tab_Project_Info.Tag)})";
                }
            }
            else if(type == ControlType.Topic)
            {
                ...
                if(row != null)
                {
                    ...
                    ObjectSubmitStatus status = (ObjectSubmitStatus)row["ti_submit_status"];
                    EnableControls(type, status != ObjectSubmitStatus.SubmitSuccess);
                }
                ...
                if(isBacked)
                {
                    btn_Topic_QTReason.Text = $"质检意见({GetAdvincesAmount(tab_Topic_Info.Tag)})";
                }
            }
            else if(type == ControlType.Subject)
            {
                ...
                if(isBacked)
                {
                    btn_JH_XM_KT_ZKT_QTReason.Text = $"质检意见({GetAdvincesAmount(tab_Subject_Info.Tag)})";
                }
            }
            else if(type == ControlType.Special)
            {
        ...
        {
            KyoButton button = sender as KyoButton;
            if(button.Name.Contains("Project"))
            {
        ...
        private void LoadDocList(object objid, ControlType type)
        {
            if(type == ControlType.Plan)
            {
                ...
                txt_Plan_AJ_Name.Text = lbl_JH_Name.Text;
            }
            else if(type == ControlType.Imp)
            {
