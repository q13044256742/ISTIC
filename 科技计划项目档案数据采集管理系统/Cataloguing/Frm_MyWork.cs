    ...
    public partial class Frm_MyWork : XtraForm
    {
        ...
        private void LoadPlanPage(TreeNode node)
        {
            if((ControlType)node.Tag == ControlType.Plan)
            {
                ...
                if(row != null)
                {
                    ...
                    LoadFileList(dgv_Plan_FileList, node.Name);
        ...
        private void LoadImpPage(TreeNode node)
        {
            ...
            if(impRow != null)
            {
                ...
                LoadFileList(dgv_Imp_FileList, ToolHelper.GetValue(impRow["imp_id"]));
                ...
                if(objId != null)
                {
                    if("lbl_Plan_Box_Add".Equals(label.Name))//新增
                    {
                        ...
                        string insertSql = $"INSERT INTO processing_box(pb_id, pb_box_number, pb_gc_fix, pb_gc_id, pb_gc_number, pb_obj_id, pb_unit_id) " +
                            $"VALUES('{Guid.NewGuid().ToString()}', '{amount + 1}', '{gch[0]}', '{gch[0] + gch[1]}', '{gch[1]}', '{objId}', '{unitCode}{_code}')";
                        SqlHelper.ExecuteNonQuery(insertSql);
                    }
        ...
        private void Cbo_Box_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            else if(comboBox.Name.Contains("Imp"))
            {
                ...
                object gcid = SqlHelper.ExecuteOnlyOneQuery($"SELECT pb_gc_id FROM processing_box WHERE pb_id='{pbId}'");
                txt_Imp_GCID.Text = ToolHelper.GetValue(gcid, string.Empty);
            }
            else if(comboBox.Name.Contains("Special"))
            {
                ...
                object gcid = SqlHelper.ExecuteOnlyOneQuery($"SELECT pb_gc_id FROM processing_box WHERE pb_id='{pbId}'");
                txt_Special_GCID.Text = ToolHelper.GetValue(gcid, string.Empty);
            }
        }
        ...
        private void LoadPageBasicInfo(TreeNode node, ControlType type)
        {
            ...
            else if(type == ControlType.Special)
            {
                ...
                LoadFileList(dgv_Special_FileList, ToolHelper.GetValue(row["imp_id"]));
        ...
        private void Tsm_DeleteRow(object sender, EventArgs e)
        {
            ...
            {
                object fileId = view.Rows[index].Cells[view.Tag + "num"].Value;
