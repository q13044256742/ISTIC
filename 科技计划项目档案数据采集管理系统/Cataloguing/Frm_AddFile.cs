    ...
    public partial class Frm_AddFile : DevExpress.XtraEditors.XtraForm
    {
        ...
        private object SaveFileInfo(DataGridViewRow row, bool isAdd)
        {
            ...
            if(isAdd)
            {
                ...
                "pfl_id, pfl_stage, pfl_categor, pfl_code, pfl_name, pfl_user, pfl_type, pfl_pages, pfl_amount, pfl_date, pfl_unit, pfl_carrier, pfl_link, pfl_file_id, pfl_obj_id, pfl_sort, pfl_remark) " +
                $"VALUES( '{primaryKey}', '{stage}', '{categor}', '{code}', '{name}', '{user}', '{type}', '{pages}', '{count}', '{date}', '{unit}', '{carrier}', '{link}', '{GetFullStringBySplit(GetLinkList(1), ",", string.Empty)}', '{parentId}', '{row.Index}', '{remark}');";
                //将备份表中的文件标记为已选取
